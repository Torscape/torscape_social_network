...
$new_password_var = hash('sha256', $new_password_var_field);
// ================================================================================
...
$enc_current_pass_var = filter_var(htmlentities($_POST['key_pass']),FILTER_SANITIZE_STRING);
$p_key_var = filter_var(htmlentities($_POST['pKey_input']),FILTER_SANITIZE_STRING);
...
if (hash('sha256', $general_current_pass_var) != $_SESSION['Password']) {
    ...
    if (empty($fullname_var) or empty($username_var)) {
        ...
        //}
         if ($stop != "1") {
         $update_info_sql = "UPDATE signup SET Fullname= :fullname_var,Username= :username_var,Password= :new_password_var,gender= :gender_var WHERE username= :session_un";
         ...
         //$update_info->bindParam(':timeid_var',$timeid_var,PDO::PARAM_STR);
         //$update_info->bindParam(':timezone_var',$timezone_var,PDO::PARAM_STR);
...
if (hash('sha256',$EditProfile_current_pass_var) != $_SESSION['Password']) {
...
if (hash('sha256', $lang_current_pass_var) != $_SESSION['Password']) {
...
if(isset($_POST['remove_key'])){
    if(hash('sha256', $enc_current_pass_var) != $_SESSION['Password']){
        $enc_save_result = "<p class='alertRed'>".lang('current_password_is_incorrect')."</p>";
    }else{
        $user_key = './keys/'.$_SESSION['id'].'.pub';
        unlink($user_key);
        if (!file_exists($user_key)) {
            $enc_save_result = "<p class='alertGreen'>".lang('changes_saved_seccessfully')."</p>";
        } else {
            $enc_save_result = "<p class='alertRed'>".lang('errorSomthingWrong')."</p>";
        }
    }
}

if(isset($_POST['pKey_input'])){
    $_SESSION['private_key'] = $p_key_var;
}
...
if (hash('sha256', $remeveA_current_pass_var) != $_SESSION['Password']) {
...
<head>
    <title>Account Settings | <?php echo lang('site_title')?></title>
    <meta charset="UTF-8">
    <meta name="keywords" content="settings,social network,social media,meet,free platform">
...
<ul class="tab" id="settings_ultab">
    ...
    <li>
        <a href="?tc=encryption" id="a_encryption">
            <p class="tablinks <?php if($tc == 'encryption'){echo 'tablinksActive';} ?>"><span class="fa fa-lock"></span>Encryption</p>
        </a>
    </li>
    <li>
...
<!--====================[ Encryption section ]======================-->
<?php case 'encryption': ?>
<div id="Encryption" class="tabcontent" style="height: auto;">
    <p align="center" id="enc_save_result"><?php echo $enc_save_result; ?></p>
    <form action="upload.php" method="post" enctype="multipart/form-data">
        <b>Public Key</b>
        <input type="file" name="publicKeyUpload" id="publicKeyUpload">
        <input value="Upload Key" type="submit">
        <br>
    </form>
    <?php
        if(file_exists('./keys/'.$_SESSION["id"].'.pub')){
            echo '<p>Public Encryption Key last updated ' . date('Y:m:d H:i:s',filemtime('./keys/'.$_SESSION["id"].'.pub')). ' UTC</p>';
        }else{
            echo '<p>No Public Encryption Key on record.</p>';
        }
    ?>
    <b>Private Key</b>
    <form action="" method="post">
        <textarea style="resize: none;" name="pKey_input" class="pKey_input" id="pKey_input" cols="75" rows="10"><?php if(isset($_SESSION['private_key'])){ echo $_SESSION['private_key'];}?></textarea>
        <br>
        <input type="submit" value="Set Private Key">
    </form>
    
    <a href="settings?tc=encryption<?php if(!isset($_GET['h'])){echo '&h=faq';}?>">How does encryption work?</a>
    <?php
        $faq = 'To set up end to end encryption you first need to generate an rsa key pair:
        On any UNIX based system you can do so by running the following command:

            openssl genrsa -out private.pem 1024 && openssl rsa -pubout -in private.pem -out public.pem

        Once you have your rsa key pair you need to upload your public key to the server:

        Click the browse button and find the file titled public.pem on your computer then press open and then Upload Key
        
        To set your private key you must copy the contents of the private.pem file to the text box and press Set Private Key:
            Your private key is never sent to or stored on the server so you will have to set your private key every time you sign in';

        $faq = nl2br($faq);
        if(isset($_GET['h'])){
            echo '<div style="background: #e9ebee; border-radius: 3px; padding: 15px;">'.$faq.'</div><br>';
        }
    ?>
    <br>
    <form action="" method="post">
         <div style="background: #e9ebee; border-radius: 3px; padding: 15px;">
         <p><input class="settings_textfield" type="password" name="key_pass" placeholder="<?php echo lang('current_password'); ?>" style="background: #fff;" /></p>
         <p style="margin: 0;"><input class="red_flat_btn" name="remove_key" type="submit" value="Delete Public Key" /></p>
         </div>
    </form>
</div>
<?php break; ?>
...
<?php default: ?>
    <div id="General" class="tabcontent" style="height: auto;">
        ...
        <form action="" method="post">
            <p>
            <p class="settings_fieldTitle"><?php echo lang('displayname'); ?> <span><?php echo lang('required'); ?></span></p>
            ...
            <p class="settings_fieldTitle"><?php echo lang('gender'); ?> <span><?php echo lang('required'); ?></span></p>
             ...
             <option <?php if($_SESSION['gender'] == "Other"){ echo "selected";} ?> ><?php echo lang('other'); ?></option>
             </select>
             </p>
        <p>
             <div style="background: #e9ebee; border-radius: 3px; padding: 15px;">
