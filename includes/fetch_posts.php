...
if (!empty($get_post_shared)) {
    $qsql = "SELECT * FROM wpost WHERE post_id=:get_post_shared";
    $query = $conn->prepare($qsql);
    $query->bindParam(':get_post_shared', $get_post_shared, PDO::PARAM_INT);
    $query->execute();

    while ($query_fetch = $query->fetch(PDO::FETCH_ASSOC)) {
        $query_fetch_post_id = $query_fetch['post_id'];
    }
    $p_title = 'Shared a <a href="/posts/post?pid='.$query_fetch_post_id.'">post</a>';
}
...
$user_path = $check_path."u/";
$hashtags_url = '/(\#)([x00-\xFF]+[a-zA-Z0-9x00-\xFF_\w]+)/';
$mentions_url = '/(\@)([x00-\xFF]+[a-zA-Z0-9x00-\xFF_\w]+)/';

//Replace with better URL RegEx
$url = '/(http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,6}(\:[0-9]+)?(\/\S*)?/';
$betterURL = '/(?:\[(.*)\]\(((?:http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,6}(?:\:[0-9]+)?(?:\/\S*)?)\)(?:\{(.*)\})*)/';
$bestURL = '/[[]*?([^\[\]]*?)][(]*?((?:http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,6}(?:\:[0-9]+)?(?:\/\S*)*?)\)[{]*?([^\{\}]*?)\}/';
$body = preg_replace($bestURL, '<a href="$2" target="_blank" title="$3">$1</a>', $post_body);
...
$body = preg_replace($mentions_url, '<a href="'.$user_path.'$2" title="$2">@$2</a>', $body);

//Text Formatting RegEx
$bold = '/\*\*(.*)\*\*/';
$body = preg_replace($bold, '<b>$1</b>', $body);

$strike = '/~~(.*)~~/';
$body = preg_replace($strike, '<strike>$1</strike>', $body);

$italics = '/\*(.*)\*/';
$body = preg_replace($italics, '<i>$1</i>', $body);

$spoiler = '/!(.*)!/';
$body = preg_replace($spoiler, '<details><summary>Spoiler!</summary>$1</details>', $body);

$underline = '/__(.*)__/';
$body = preg_replace($underline, '<u>$1</u>', $body);

$header_three = '/\$\$\$(.*)\$\$\$/';
$body = preg_replace($header_three, '<h3>$1</h3>', $body);

$header_two = '/\$\$(.*)\$\$/';
$body = preg_replace($header_two, '<h2>$1</h2>', $body);

$header_one = '/\$(.*)\$/';
$body = preg_replace($header_one, '<h1>$1</h1>', $body);

if($query_fetch_verify == 1){$verified = "<span style='color: #03A9F4;' data-toggle='tooltip' data-placement='top' title='".lang('verified_page')."' class='fa fa-check-circle verifyUser'></span>";}else{$verified = '';}
...
$body = nl2br("$body");
    ...
    <td>
    <a href='".$check_path."u/$query_fetch_username' class='username_OF_postLink'>$query_fetch_fullname</a>$verified<br/>
    <a href='".$check_path."posts/post?pid=".$get_post_id."' title='".date('d M, Y g:i a', $get_post_time)."' class='username_OF_postTime'>$timeago</a>
    </td>
    <td>
    <div class='dropdown'>
    ...
    ";
    if ($get_author_id == $_SESSION['id'] or $_SESSION['admin'] > 0) {
            echo "
...
<a href=\"".$check_path."posts/post?pid=$shP_id\" title='Test' class=\"username_OF_postTime\">$shP_timeago</a>
</td>
</tr>
</tbody></table>
</div><div id='postTitle_$get_post_id'>";
...
//Replace Alert
//sharePost('$get_post_id','$check_path')
echo "<a href='javascript:void(0);' onclick=\"alert('Post Sharing is currently disabled.')\" class='post_like_comment_shareA' data-toggle='tooltip' data-placement='top' title='".lang('share_now')."'><span class=\"fa fa-share-alt\"></span></a>
