...
if ($getHintCount > 0) {
	while ($getHint_row = $getHint->fetch(PDO::FETCH_ASSOC)) {
		...
		if (strlen($lastMsg) > 20) {
			if(strlen($lastMsg)>20){$lastMsg = substr($lastMsg, 0,20)."...";}
		}
...
$image_url = '/(http(s?):)([\/|.|\w|\s|-])*\.(?:jpg|jpeg|gif|png)/';
$apos = '/(&#39;)+/';
$message_body = preg_replace($apos, '\'', $message_body);
$message_body = preg_replace($hashtags_url, '<a href="'.$hashtag_path.'$2" title="#$2">#$2</a>', $message_body);
$message_body = preg_replace($mentions_url, '<a style="color: #2F4F4F; text-decoration: none;" href="'.$user_path.'$2" title="@$2">@$2</a>', $message_body);
$message_body = preg_replace($image_url, '<a href="$0" title="$0" target="_blank"><img witdh="200px" height="300px" style="border-radius: 5%;" src="$0" alt="$0" /></a><br>', $message_body);
...
<table class='m_msgTable' data-count='".$getMsgsCout."' style='direction:".$userDir.";'>
	<tr>
		".$msgUserPhoto."
		<td>
			<div class='".$m_msgU."' data-is='".date("d M, Y - g:i a",$msgs_row['m_time'])."' style='word-wrap: break-word; direction:".lang('dir').";'>
			<p style='margin:0;display: inline;'>".$message_body."</p>
			</div>
		</td>
	</tr>
</table>
...
//Old Message: Incase anything breaks
