...
switch(getOS()){
                case 'MacOS':
                    var emojiShortcut = "More Emojis - CTRL + CMD + SPACE";
                    break;
                case 'Windows':
                    var emojiShortcut = "More Emojis - WIN + PERIOD";
                    break;
                case 'Linux':
                    var emojiShortcut = 'More Emojis - CTRL + PERIOD';
                    break;
                case 'iOS':
                    var emojiShortcut = '';
                    break;
...
$.ajax({
    ...
    success: function(done){
        if (done == 'yes') {
            ...
            $('#'+pid).show();
            alert('Action denied! You are not allowed to preform this action');
        }
    }
});
}
...
if($('#em_'+cid).attr('data-emtog') == '0'){
    ...
    if(checkData.length == 0){
        ...
        success:function(data){
        $('#em_'+cid).html("<div'><span class='toTopArrow' style='position: absolute; top: -10px;"+"<?php echo lang('float2'); ?>"+":8px;'></span><p style='position: absolute; border: none; color: #C0C0C0; font-size: xx-small; padding-right: 5px; right: 0; bottom: 0;'>"+emojiShortcut+"</p></div>"+data);
        },
        error:function(){
            alert('Error, something went wrong!');
        }
...
if($('#emBox').attr('data-emtog') == '0'){
    ...
    if(checkData.length == 0){
        ...
        success:function(data){
        $('#emBox').html("<div><span class='toBottomArrow' style='position: absolute; bottom: -10px;"+"<?php echo lang('float2'); ?>"+":8px;'></span><p style='position: absolute; color: #C0C0C0; font-size: xx-small; padding-right: 5px; right: 0; bottom: 0;'>"+emojiShortcut+"</p></div>"+data);
        },
        error:function(){
            alert('Error, something went wrong!');
        }
...
////////////////////////////////////
function fcomment(txtfid, txt){
    var str = txt || '';
    $('#inputComm_'+txtfid).val(txt);
...
$.ajax({
    ...
    $('#postNotify_'+fid).html("<p class='postNotify'><img src='<?php echo $check_path; ?>imgs/loading_video.gif' style='width:20px;box-shadow: none;height: 20px;'> Sending your feedback...</p>");
    });
    },
    success:function(data){
    $('#postNotify_'+fid).slideDown(300,function(){
...
beforeSend:function(){
$('#report_submit').hide();
$('#SubLog').html("<p><img src='<?php echo $checkDir; ?>imgs/loading_video.gif' style='width:20px;box-shadow: none;height: 20px;'> Sending your feedback...</p>");
},
success:function(data){
if (data == "done") {
