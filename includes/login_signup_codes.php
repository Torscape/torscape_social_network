...
$APIKey = 'RYyHdcLVlIqSjyzoRItYV4sVZ0LIQs';
...
echo "<p class='alertRed'>".lang('enter_username_to_login')."</p>";
    ...
    $chekPwd = $conn->prepare("SELECT * FROM signup WHERE Username = :username");
    ...
    }elseif (hash('sha256',$password) != $rPassword) {
    ...
    $loginsql = "SELECT * FROM signup WHERE (Username= :username) AND Password= :rPassword";
...
$signup_password = hash('sha256', $signup_password_var);
// ================================================================================
...
$signup_timezone = filter_var(htmlentities($_POST['tz']),FILTER_SANITIZE_STRING);
...
}elseif ($signup_genderVar == lang('other')) {
    $signup_gender = "Other";
    $userphoto = "user-male.png";
}else{
    $signup_gender = "Male";
...
//}
    ...
    $signup_admin = "1";   
    $signupsql = "INSERT INTO signup (id,Fullname,Username,Password,Userphoto,gender,timezone,language,admin)
    VALUES( :signup_id, :signup_fullname, :signup_username, :signup_password, :userphoto, :signup_gender, :signup_timezone,:signup_language,:signup_admin)";
    ...
    $query->bindParam(':signup_timezone', $signup_timezone, PDO::PARAM_STR);
    $query->bindParam(':signup_language', $signup_language, PDO::PARAM_STR);
    $query->bindParam(':signup_admin', $signup_admin, PDO::PARAM_INT);
    $query->execute(); 
    }else{
    $signupsql = "INSERT INTO signup (id,Fullname,Username,Password,Userphoto,gender,timezone,language)
    VALUES( :signup_id, :signup_fullname, :signup_username, :signup_password, :userphoto, :signup_gender, :signup_timezone,:signup_language)";
    $query = $conn->prepare($signupsql);
    $query->bindParam(':signup_id', $signup_id, PDO::PARAM_INT);
    $query->bindParam(':signup_fullname', $signup_fullname, PDO::PARAM_STR);
    $query->bindParam(':signup_username', $signup_username, PDO::PARAM_STR);
    ...
    $main_user = 'Torscape';
    $main_id_sql = "SELECT id FROM signup WHERE (Username= :main_username)";
    $query = $conn->prepare($main_id_sql);
    $query->bindParam(':main_username', $main_user, PDO::PARAM_STR);
    $query->execute();
    while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
        $mainaccount_id = $row['id'];
    }
    $followsql = "INSERT INTO follow (uf_one, uf_two)
    VALUES( :ufone, :uftwo)";
    $query = $conn->prepare($followsql);
    $query->bindParam(':ufone', $signup_id, PDO::PARAM_INT);
    $query->bindParam(':uftwo', $mainaccount_id, PDO::PARAM_INT);
    $query->execute(); 

    // ============================ Update Follow Count Official Account ============================
    $followers_sql = "SELECT id FROM follow WHERE uf_two=:main";
    $followers = $conn->prepare($followers_sql);
    $followers->bindParam(':main',$mainaccount_id,PDO::PARAM_INT);
    $followers->execute();
    $followers_num = $followers->rowCount();
    $UpdateFollowersSql = "UPDATE signup SET followers=:followers_num WHERE id=:main";
    $UpdateFollowers = $conn->prepare($UpdateFollowersSql);
    $UpdateFollowers->bindParam(':followers_num',$followers_num,PDO::PARAM_INT);
    $UpdateFollowers->bindParam(':main',$mainaccount_id,PDO::PARAM_INT);
    $UpdateFollowers->execute();

    // ============================ TODO: Auto Send Welcome Message ============================


    // ============================ Login Code after Signup ============================
    $loginsql = "SELECT * FROM signup WHERE (Username= :signup_username) AND Password= :signup_password";
    $query = $conn->prepare($loginsql);
    $query->bindParam(':signup_username', $signup_username, PDO::PARAM_STR);
    //$query->bindParam(':signup_email', $signup_email, PDO::PARAM_STR);
    $query->bindParam(':signup_password', $signup_password, PDO::PARAM_STR);
