...
$db_password = hash('sha256', $db_password_var);
// ================================================================================
...
$uInfo = $conn->prepare("SELECT id,Fullname,Username,Password,admin FROM signup WHERE Username = :ed");
...
if (isset($_POST['submit_uInfo'])) {
    if (empty($db_fullname) or empty($db_username)) {
    ...
    if ($stop != "1") {
        ...
        $update = $conn->prepare("UPDATE signup SET Fullname = :db_fullname,Username = :db_username,admin = :db_admin WHERE Username = :ed");
        }else{
        $update = $conn->prepare("UPDATE signup SET Fullname = :db_fullname,Username = :db_username,Password = :db_password,admin = :db_admin WHERE Username = :ed");
        }
