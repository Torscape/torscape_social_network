...
<head>
    <title><?php echo $row_username; ?> | <?php echo lang('site_title')?></title>
    <meta charset="UTF-8">
    <meta name="description" content="<?php echo $row_bio; ?>">
    <meta name="keywords" content="social network,social media,meet,free platform">
...
<!--=============================[ Container ]=====================================-->
        ...
        <div class="profile_menu">
            <div class="profile_menu_details" style="display:inline-flex;">
                ...
                $followerPost = 1;
                $privatePost = 2;
                $posts_num_sql = "SELECT post_id FROM wpost WHERE author_id=:row_id AND p_privacy=:privacy";
                $posts_num = $conn->prepare($posts_num_sql);
                $posts_num->bindParam(':row_id',$row_id,PDO::PARAM_INT);
                $posts_num->bindParam(':privacy',$publicPost,PDO::PARAM_INT);
                $posts_num->execute();
                $public_posts_num_int = $posts_num->rowCount();
                //=====================================================================
                $following_num_sql = "SELECT id FROM follow WHERE uf_one=:my_id AND uf_two=:row_id";
                $following_num = $conn->prepare($following_num_sql);
                $following_num->bindParam(':my_id',$myId,PDO::PARAM_INT);
                $following_num->bindParam(':row_id',$row_id,PDO::PARAM_INT);
                $following_num->execute();
                $following_count = $following_num->rowCount();
                if($following_count > 0){
                    $posts_num_sql = "SELECT post_id FROM wpost WHERE author_id=:row_id AND p_privacy=:privacy";
                    $posts_num = $conn->prepare($posts_num_sql);
                    $posts_num->bindParam(':row_id',$row_id,PDO::PARAM_INT);
                    $posts_num->bindParam(':privacy',$followerPost,PDO::PARAM_INT);
                    $posts_num->execute();
                    $follower_posts_num_int = $posts_num->rowCount();
                }else{
                    $follower_posts_num_int = 0;
                }
                ...
                if($myId == $row_id){
                    ...
                    $posts_num = $conn->prepare($posts_num_sql);
                    $posts_num->bindParam(':row_id',$row_id,PDO::PARAM_INT);
                    $posts_num->bindParam(':privacy',$privatePost,PDO::PARAM_INT);
                    $posts_num->execute();
                    $private_posts_num_int = $posts_num->rowCount();
                }else{
                    $private_posts_num_int = 0;
                }
                //=====================================================================
                $posts_num_int = $public_posts_num_int + $follower_posts_num_int + $private_posts_num_int;
                //=====================================================================
        ...
        <div class="profile_picture_img profile_ppicture" style="<?php echo $profile_pic_border_var;?>">
            ...
            $onionURL = '/(?:https?:\/\/)?(?:www)?(\S*?\.onion)\b/';
            $universal = '/(https?:\/\/)?([\da-z\.-]+\.[a-z\.]{2,6}|[\d\.]+)([\/:?=&#]{1}[\da-z\.-]+)*[\/\?]?$/i';
            $website_row = preg_replace($universal, '<a href="$0" target="_blank" title="$0">Website</a>', $row_website);
            ?>
        <div class="profile_picture">
            ...
            //echo strtok($firstname, " "); //First Name Only
            echo $row_fullname;
            ?>
            ...
            if(empty($row_school) && empty($row_work0) && empty($row_work) && empty($row_country) && empty($row_birthday) && empty($row_gender) && empty($row_website)){echo "<p>".lang('nothingToShow')."</p>";}
            if (!empty($row_school)){echo "<tr><td class='user_info_tdi'><i title=\"Education\" class=\"fa fa-graduation-cap\"></i></td><td>".lang('studies')." $row_school<td></tr>";}
            if (!empty($row_work0) || !empty($row_work)){echo "<tr><td title=\"Job\" class='user_info_tdi'><i class=\"fa fa-briefcase\"></i></td><td>".lang('working')." $row_work0 ".lang('at')." $row_work<td></tr>";}
            if (!empty($row_country)){echo "<tr><td class='user_info_tdi'><i title=\"Country\" class=\"fa fa-map-marker\"></i></td><td>".lang('lives_in')." $row_country<td></tr>";}
            if (!empty($row_birthday)){echo "<tr><td class='user_info_tdi'><i title=\"Birthday\" class=\"fa fa-calendar\"></i></td><td>".lang('born_on')." $row_birthday<td></tr>";}
            if (!empty($row_gender)){echo "<tr><td class='user_info_tdi'><i title=\"Gender\" class=\"fa fa-transgender-alt\"></i></td><td> $row_gender<td></tr>";}
            if (!empty($website_row)){echo "<tr><td class='user_info_tdi'><i title=\"Website\" class=\"fa fa-globe\"></i></td><td>$website_row<td></tr>";}
            ?>
            </table>
        </div>
        <div class="user_info">
...
<!--=============================================myphotos_section==================================================-->
            ...
            <div class="userPhotosProfile">
                <a target="_blank" href="../imgs/<?php echo $fetch_post_img; ?>"><img src="../imgs/<?php echo $fetch_post_img; ?>" /></a>
            </div>
            <?php
            }   
            ?>
...
if ($_SESSION['id'] == $row_id) {
    ...
    $following_paragraph = lang('uProf_followingTitle1')." $row_fullname ".lang('uProf_followingTitle2');
}
?>
